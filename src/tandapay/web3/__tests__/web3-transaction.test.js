...
describe('Phase 1.3: Transaction Functions Migration', () => {
  ...
  describe('Legacy Wrapper Compatibility', () => {
    test('legacy functions maintain old API format', async () => {
      // Import legacy functions
      const { transferTokenLegacy, estimateTransferGasLegacy, getTokenInfoLegacy } = require('../web3');

      // Test transferTokenLegacy
      const transferResult = await transferTokenLegacy(
        mockETHToken,
        '0x1234567890123456789012345678901234567890123456789012345678901234',
        '0xrecipient123456789012345678901234567890',
        '1.0',
      ...
      expect(transferResult).toHaveProperty('success');
      expect(transferResult.success).toBe(true);
      expect(transferResult).toHaveProperty('txHash');
      expect(transferResult.txHash).toBe('0xtest456');

      // Test estimateTransferGasLegacy
      const gasResult = await estimateTransferGasLegacy(
        mockETHToken,
        '0xsender123456789012345678901234567890',
        '0xrecipient123456789012345678901234567890',
        '1.0',
      ...
      expect(gasResult).toHaveProperty('success');
      expect(gasResult.success).toBe(true);
      expect(gasResult).toHaveProperty('gasEstimate');
      expect(gasResult).toHaveProperty('gasPrice');

      // Test getTokenInfoLegacy
      const tokenResult = await getTokenInfoLegacy(
        '0xa0b86a33e6dc391d0165b8b7e0e5a53ff8d6eeb6',
        'sepolia'
      );
      ...
      expect(tokenResult).toHaveProperty('tokenInfo');
      expect(tokenResult.tokenInfo).toHaveProperty('symbol');
      expect(tokenResult.tokenInfo?.symbol).toBe('USDC');
    });
  });
});
